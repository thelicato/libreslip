...
Schema 8 mode, paired-Server destination, Client outbox and Server inbox tables are deliberately excluded until the Task 14 archive extension; restore therefore keeps the destination device's local mode and networking data. Client access tokens, Server-side token hashes and the Server private key remain excluded. Full backups preserve each reusable item's Server-delivery flag. Existing schema 5, 6 and 7 snapshots are accepted because their archived table inventory is identical, and a missing item flag defaults to included. The format is otherwise intentionally strict, and an incompatible format or portable-table schema must use a new version.
