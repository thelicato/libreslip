...
| `database.json` | Full backup only | Database schema version 6, or accepted legacy version 5, and a consistent snapshot of categories, items, drafts, draft lines, tickets, ticket lines, counters, print jobs and order feature settings. Binary print payloads use Base64. |
...
Schema 6 mode, destination, outbox and server inbox tables are deliberately excluded until the Task 13 archive extension; restore therefore keeps the destination device's local mode and networking data. Pairing secrets and the Server private key remain excluded. Existing schema 5 snapshots are accepted because their archived table inventory is identical. The format is otherwise intentionally strict, and an incompatible format or portable-table schema must use a new version.
