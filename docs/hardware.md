...
Overview reports whether LibreSlip's RFCOMM socket is connected and identifies the selected bonded device. This is different from Android pairing: a paired printer can still be disconnected. Refresh checks current socket state without reconnecting or sending data.
...
| Reconnect and explicit reprint | Hands-on workflow completed; storage tests verify that reprint adds an attempt without adding a ticket. |
...
| Connection dashboard | Controller and widget tests cover connected device, disconnected state, refresh and state clearing. |
