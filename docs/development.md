...
PDF tickets are rendered locally with the same bundled fonts and passed to the Android system share sheet. Version 0.11.0 adds only the Server receipt side of the [local order protocol](network-protocol.md). Client mode does not upload or deliver tickets yet and chooses no destination on the user's behalf.
...
Images are written under `build/previews`. This optional capture is skipped during normal tests; it is a review aid rather than a pixel-perfect golden assertion. The task 11 review set adds the Italian phone Server inbox to the English phone/tablet Overview, scrolled Italian dashboard, disconnected Compose, ticket, Settings and portability screens.
