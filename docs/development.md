...
Images are written under `build/previews`. This optional capture is skipped during normal tests; it is a review aid rather than a pixel-perfect golden assertion. The task 9 review set includes English phone/tablet Overview renders, a scrolled Italian phone dashboard with representative ticket counts and an item breakdown, and the disconnected Compose guidance alongside the existing ticket, Settings and portability screens.
