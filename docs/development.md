...
Images are written under `build/previews`. This optional capture is skipped during normal tests; it is a review aid rather than a pixel-perfect golden assertion. The task 8 review set includes English phone/tablet Overview renders and a scrolled Italian phone dashboard with representative ticket counts and an item breakdown to the existing Compose, ticket, Settings and portability screens.
