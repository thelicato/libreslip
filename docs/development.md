...
Images are written under `build/previews`. This optional capture is skipped during normal tests; it is a review aid rather than a pixel-perfect golden assertion. The task 14 review set includes the complete English item-total dialog alongside the Italian Server Orders tab, English Server Settings tab, Client pairing, Overview, Compose, item, ticket, Settings and portability screens.
