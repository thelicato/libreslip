...
Images are written under `build/previews`. This optional capture is skipped during normal tests; it is a review aid rather than a pixel-perfect golden assertion. The task 13 review set includes the Italian Server Orders tab, English Server Settings tab and Italian local-only item state alongside Client pairing, Overview, Compose, ticket, Settings and portability screens.
