...
def main():
    ...
    source = destination / 'LibreSlip-task-11-server-inbox.zip'
    ...
    if args.include_apk:
        built = ROOT / 'build/app/outputs/flutter-apk/app-release.apk'
        apk = destination / 'LibreSlip-task-11-server-inbox-preview.apk'
    ...
    (destination / 'LibreSlip-task-11-server-inbox-SHA256SUMS.txt').write_text(''.join(checksums))
