...
def main():
    ...
    source = destination / 'LibreSlip-task-13-server-workflow.zip'
    ...
    if args.include_apk:
        built = ROOT / 'build/app/outputs/flutter-apk/app-release.apk'
        apk = destination / 'LibreSlip-task-13-server-workflow-preview.apk'
    ...
    (destination / 'LibreSlip-task-13-server-workflow-SHA256SUMS.txt').write_text(''.join(checksums))
